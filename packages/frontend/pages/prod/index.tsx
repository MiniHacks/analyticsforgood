...
const HEADINGS = ["ID", "Name", "OTIF", "OT + Partial", "Avg Fulfillment"];
    ...
    .map((procut: ProductDisplayType) => {
      return (
        <Tr
          ...
          onClick={() => router.push(`/prod/${procut.id}`)}
        >
          <TableDataCell color={"gray.400"}>{procut.id}</TableDataCell>
          <TableDataCell>{CROP_MAPPING[procut.id]}</TableDataCell>
    ...
    <VStack ml={"200px"} mt={24} px={8} alignItems={"start"} width={"100%"}>
      ...
      <HStack justifyContent={"space-between"} width={"100%"}>
        <VStack alignItems={"start"} spacing={2} pb={4}>
          <Heading fontSize={48}>Products</Heading>
        </VStack>
        <Menu>
          <MenuButton
            as={Button}
